...
public interface ParticipationRepository extends JpaRepository<Participation, Long>, ParticipationRepositoryCustom {
    ...
    @Query(value =
            "SELECT " +
                    "COALESCE(COUNT(CASE WHEN p.status = 'PENDING' THEN 1 END), 0) as pendingCount, " +
                    "COALESCE(COUNT(CASE WHEN p.status = 'APPROVED' THEN 1 END), 0) as approvedCount, " +
                    "COALESCE(COUNT(CASE WHEN p.status = 'COMPLETED' THEN 1 END), 0) as completedCount, " +
                    "COALESCE(COUNT(CASE WHEN pr.reward_status = 'PENDING' THEN 1 END), 0) as pendingRewards, " +
                    "COALESCE(COUNT(CASE WHEN pr.reward_status = 'PAID' THEN 1 END), 0) as paidRewards " +
                    "FROM participations p " +
                    "LEFT JOIN participant_rewards pr ON p.id = pr.participation_id " +
                    "WHERE p.post_id = :postId",
            nativeQuery = true)
    List<Object[]> getPieChartStatsByPostId(@Param("postId") Long postId);
    ...
    @Query(value =
            ...
            nativeQuery = true)
    List<Object[]> getParticipationStatsByPostId(@Param("postId") Long postId);
}
