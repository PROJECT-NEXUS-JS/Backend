...
public interface ParticipationRepository extends JpaRepository<Participation, Long> {
    ...
    @Query("SELECT p FROM Participation p JOIN FETCH p.user WHERE p.post.id = :postId AND p.status = :status" +
    ...
    @Query("SELECT p FROM Participation p JOIN FETCH p.post WHERE p.user.id = :userId AND p.status = :status ORDER BY p.appliedAt DESC")
    List<Participation> findByUserIdAndStatusWithPost(@Param("userId") Long userId, @Param("status") ParticipationStatus status);

    // 게시글 작성자의 참여 관리 목록
    @Query("SELECT p FROM Participation p JOIN FETCH p.user WHERE p.post.createdBy = :creatorId AND p.status = :status ORDER BY p.appliedAt ASC")
    ...
    // 특정 기간 참여 신청 조회
    @Query("SELECT p FROM Participation p WHERE p.appliedAt BETWEEN :startDate AND :endDate ORDER BY p.appliedAt DESC")
